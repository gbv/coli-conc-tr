...
\lohead{% Kopfzeile linke Seite
  ...
  \draw [fill,GBVblue] (0mm,-6.4mm) rectangle +(\paperwidth,-11mm);
  \tikz[page position=current page.north east]
  \node [anchor=east,text=white] at (-30mm,-11.9mm) { \upshape\textsf{$series$ $number$} };
}
\rehead{% Kopfzeile rechte Seite
  \tikz[page position=current page.north west]
  \draw [fill,GBVblue] (0mm,-6.4mm) rectangle +(\paperwidth,-11mm);
  \tikz[page position=current page.north west]
  \node [anchor=west,text=white] at (30mm,-11.9mm) { \upshape\textsf{$title$} };
}
...
\cfoot{% Fußzeile: Linie
  ...
  \draw[draw=GBVblue,line width=0.5pt] (0,1cm) to +(\paperwidth,0);
}
\lofoot{% Fußzeile: Seitezahl rechts
  \tikz[page position=current page.south east]
  \node[anchor=south east,fill=GBVblue,minimum height=0.25cm] at (-2.5cm,1cm) {\pagemark};
}
\refoot{% Fußzeile: Seitezahl links
  \tikz[page position=current page.south west]
  \node[anchor=south west,fill=GBVblue,minimum height=0.25cm] at (2.5cm,1cm) {\pagemark};
}
